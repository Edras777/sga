...
const getPrestamoLibros = async (req, res = response) => {
  try {
    const prestamo_libros = await PrestamoLibros.find().populate([
      "estudiante",
      "libro",
      "docente",
    ]);
...
const getPrestamoLibro = async (req, res = response) => {
  try {
    const prestamo_libro = await PrestamoLibros.findById(
      req.params.id
    ).populate(["estudiante", "libro", "docente"]);
...
const actualizarPrestamoLibro = async (req, res = response) => {
  try {
    ...
    const prestamo_libro = await PrestamoLibros.findByIdAndUpdate(id, data, {
      new: true,
    }).populate(["estudiante", "libro", "docente"]);
...
const getLibroByCodigo = async (req, res = response) => {
  try {
    ...
    const libro = await Libro.findOne({ codigo: codigo });
