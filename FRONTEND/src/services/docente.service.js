...
const getDocenteByCUI = async CUI => {
  const response = await axios.get(`${API_URL}/docentes/cui/${CUI}`);
  return response.data;
};
...
const updateDocente = async (data, token) => {
  ...
  if (response.status === 200 || response.status === 201) {
    ToastChakra(
      'DOCENTE MODIFICADO',
      'El registro ha sido modificada correctamente',
      'success',
      1500,
      'bottom'
    );
