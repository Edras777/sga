...
import ModalRegistrarPago from './ModalRegistrarCalificacion';
import ModalGenerarBoleta from './ModalGenerarBoleta';
import ModalEditarPago from './ModalEditarCalificacion';
...
const Calificaciones = () => {
  ...
  const { pagos, isLoading, isError, message } = useSelector(
    state => state.pagos
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => (
        <div>
          <Link to={`/estudiantes/${row?.estudiante?._id}`}>
            <Text
              fontSize="xs"
              color="blue.500"
              _hover={{
