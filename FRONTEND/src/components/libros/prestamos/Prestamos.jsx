...
const PrestamoLibros = () => {
  ...
  const { prestamo_libros, isLoading, isError, message } = useSelector(
    state => state.prestamo_libros
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row =>
        row.estudiante
          ...
          : row.docente?.nombres + ' ' + row.docente?.apellidos,
      sortable: true,
      cellExport: row =>
        row.estudiante
          ? row.estudiante?.nombres + ' ' + row.estudiante?.apellidos
    ...
    },
    {
      name: 'FECHA PRESTAMO',
      selector: row =>
        Moment(row.fecha_prestamo).format('DD/MM/YYYY - hh:mm:ss A'),
      sortable: true,
      cellExport: row =>
        Moment(row.fecha_prestamo).format('DD/MM/YYYY - hh:mm:ss A'),
      resizable: true,
        ...
        <Stack spacing={4} direction="row" justifyContent="space-between" p={4}>
          <HStack spacing={4} direction="row">
            <Link to={'/libros/prestamos/agregar'}>
              <Tooltip
                hasArrow
                label="Registrar nuevo prestamo"
                placement="auto"
              >
                <IconButton
