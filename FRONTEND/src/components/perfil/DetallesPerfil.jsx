...
const DetallesPerfil = ({ usuario }) => {
  ...
  const { persona, isLoading, isError, message } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getPersona(usuario._id));

    return () => {
      dispatch(reset());
    };
  }, [user, navigate, isError, message, dispatch, usuario._id]);
          ...
          >
            <Stack direction="column" spacing={4} justifyContent="stretch">
              ...
              >
                ...
                <Badge
                  bg={
                    persona.rol === 'ADMIN_ROLE' ? 'messenger.600' : 'red.600'
                  }
                  variant="solid"
                  w={36}
                  textAlign="center"
