...
import { FaFileInvoiceDollar, FaNotesMedical } from 'react-icons/fa';
...
const Estudiantes = () => {
  ...
  const { estudiantes, isLoading, isError, message } = useSelector(
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      cellExport: row => row.CUI,
      resizable: true,
    },
    {
      name: 'NOMBRES PADRES O ENCARGADO',
      selector: row => row.nombre_padres,
      sortable: true,
      cellExport: row => row.nombre_padres,
      resizable: true,
    },
    {
      name: 'GRADO',
    ...
    },
    {
      ...
      cell: row => (
        <div>
          <Link
            to={{
              pathname: '/estudiantes/calificacion/' + row._id,
              search: `?grado=${row.grado._id}`,
            }}
          >
            <Tooltip hasArrow label="Calificaciones" placement="auto">
              <IconButton
                aria-label="Ver"
                icon={<FaNotesMedical />}
                fontSize="2xl"
