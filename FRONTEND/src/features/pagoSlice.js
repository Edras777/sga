...
import pagoService from '../services/pago_ceba.service';
...
export const getPago = createAsyncThunk('pago/get', async (id, thunkAPI) => {
  try {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(deletePago.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.pagos = state.pagos.filter(pay => pay._id !== action.payload._id);
      })
      ...
      .addCase(updatePago.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.pagos = state.pagos.map(pay =>
